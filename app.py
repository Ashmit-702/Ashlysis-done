...
{{
  ...
  "study_plan": {{
    ...
    "days": [ {{"day":1,"focus":"topic","priority":"HIGH","hours":3,"tasks":["task1"]}} ],
...
def analyze():
    ...
    try:
        ...
        return jsonify({
            'clusters': clusters, 'predictions': enrichment.get('predictions', []),
            'study_plan': enrichment.get('study_plan', {}), 'paper_pattern': enrichment.get('paper_pattern', {}),
            'user_name': user_name, 'university': UNIVERSITY_DB.get(university, {}).get('name', ''),
            'subject': subject.upper(), 'stats': {'papers': len(all_papers_text), 'total_questions': len(all_questions), 'clusters': len(clusters)}
        })
    finally:
